...
- Shared types (`slopcc-common`) implemented: `Span`, `BytePos`, `FileId` (6 unit tests).
- Total: 44 passing tests across workspace.

## Next Up
- [ ] P1: implement preprocessor (`slopcc-pp`) — `#define`, `#include`, `#if`/`#ifdef`/`#ifndef`, macro expansion, `#` / `##` operators
- [ ] P2: add `Diagnostic` type to `slopcc-common` + error reporting infrastructure
- [ ] P3: add `SourceMap` to `slopcc-common` (FileId + offset → file/line/column)
...
- [ ] DRIVER-1: GCC-compatible CLI argument parsing (`clap`)
- [ ] DRIVER-2: pipeline orchestration (lex → preprocess → parse → sema → codegen)
- [ ] DRIVER-3: external linker invocation
...
## Last Updated
- 2026-02-15 by opencode-agent in feature/lex-core
