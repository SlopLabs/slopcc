use slopcc_common::{source::FileId, span::Span};
...
impl<'src> Lexer<'src> {
    #[must_use]
    pub fn new(src: &'src [u8], file: FileId) -> Self {
        Self {
    ...
    #[must_use]
    pub fn tokenize(src: &'src [u8], file: FileId) -> Vec<Token> {
        let mut lexer = Self::new(src, file);
        let mut out = Vec::new();
        loop {
    ...
    pub fn lex_header_name(&mut self) -> Token {
        ...
        match self.cursor.peek() {
            ...
            None => Token::new(TokenKind::Eof, Span::at(self.file, self.pos32())),
        }
    }
    ...
    fn make_token(&self, start: usize, kind: TokenKind) -> Token {
        ...
        Token::new(
            kind,
            Span::new(self.file, start as u32, self.cursor.pos() as u32),
        )
    }
...
mod tests {
    ...
    fn lexes_whitespace_with_span() {
        let mut lexer = Lexer::new(b" \t\r\x0B\x0C", fid());
        let token = lexer.next_token();
    ...
    fn lexes_unterminated_block_comment_as_comment() {
        let tokens = Lexer::tokenize(b"/* not closed", fid());
    ...
    fn lexes_identifiers() {
        let tokens = Lexer::tokenize(b"foo _bar x123", fid());
        let kinds: Vec<_> = tokens.iter().map(|token| token.kind).collect();
        assert_eq!(
            kinds,
            vec![
    ...
    fn lexes_pp_numbers() {
        ...
        let tokens = Lexer::tokenize(src, fid());
    ...
    fn lexes_string_literals_and_prefixes() {
        let src =
            b"\"hello\" \"with \\\"escape\\\"\" L\"wide\" u8\"utf8\" u\"utf16\" U\"utf32\" \"\"";
        let tokens = Lexer::tokenize(src, fid());
    ...
    fn lexes_header_names() {
        let mut angle = Lexer::new(b"<stdio.h>", fid());
        assert_eq!(angle.lex_header_name().kind, TokenKind::HeaderName);

        let mut quote = Lexer::new(b"\"myheader.h\"", fid());
    ...
    fn pp_numbers_greedy_sign_exponents() {
        let mut lexer = Lexer::new(b"0x1p+3 1e-2", fid());
