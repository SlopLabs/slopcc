...
```rust
Lexer::new(src: &[u8]) -> Lexer            // Create lexer over source bytes
Lexer::next_token(&mut self) -> Token       // Consume next preprocessing token
Lexer::tokenize(src: &[u8]) -> Vec<Token>   // Tokenize entire input (ends with Eof)
Lexer::lex_header_name(&mut self) -> Token  // Lex <...> or "..." header name (#include context)
...
- `slopcc-common` — `Span` and `BytePos` for source location tracking.
